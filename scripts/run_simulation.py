...
parser.add_argument('--num_simulations', type=int, default=1,
                    help='number of simulations to run in sequence')
...
configuration, num_simulations, plot_config = args.configuration, \
                                              args.num_simulations, \
                                              args.plot_config

seed, scale, hours = args.seed, \
                     args.scale, \
                     args.hours
...
for _ in range(num_simulations):
    # eventually change seed if there are multiple simulations
    if num_simulations > 1:
        seed = np.random.randint(1000, 99999999, dtype=int)
    ...
    os.system(f"cd ..; "
              f"python get_csv_simulations.py --simulation_name {configuration}{seed};"
              f"cd scripts;")

    # computes PC on the simulation
    os.system(f"python pc.py --seed {seed} --simulation_name {configuration}{seed}")
